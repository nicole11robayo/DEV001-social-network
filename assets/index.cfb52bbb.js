 ...
 * limitations under the License.
 */
async function updateProfile(user2, { displayName, photoURL: photoUrl }) {
...
const mintCookieFactory = (url) => async (user2) => {
        ...
        try {
          switch (o.getLastErrorCode()) {
            case ErrorCode.NO_ERROR:
              ...
              C("Connection", "XHR received:", JSON.stringify(n2)), i(n2);
              break;
            ...
            case ErrorCode.HTTP_ERROR:
              ...
              if (C("Connection", 'RPC "' + t2 + '" failed with status:', s2, "response text:", o.getResponseText()), s2 > 0) {
                ...
                if (n3 && n3.status && n3.message) {
                  const t4 = function(t5) {
                    ...
                    return Object.values(B).indexOf(e3) >= 0 ? e3 : B.UNKNOWN;
                  }(n3.status);
                  r(new L(t4, n3.message));
                } else
...
const database = La(app);
const user = () => {
  auth.onAuthStateChanged((us2) => {
    if (us2) {
      localStorage.setItem("uidUsuario", us2.uid);
      localStorage.setItem("emailUser", us2.email);
      localStorage.setItem("displayName", us2.displayName);
      console.log(us2.displayName);
...
const profileUpdate = (displayName) => updateProfile(auth.currentUser, {
  displayName
});
...
const registerEmailAndPassword = (email, contrase\u00F1a) => {
...
const newUserCollection = (userID, usuario, email, contrase\u00F1a) => {
  ml(sa(database, "users", userID), {
    user: usuario,
    correo: email,
    password: contrase\u00F1a,
    id: userID
...
const newPostCollection = (post, name2, uid) => {
  pl(ea(database, "posts"), {
    post,
    name: name2,
    uid,
    likes: [],
...
const collectionFirebase = ea(database, "posts");
const order = () => Mh(collectionFirebase, Lh("dateCreated", "desc"));
const onGetUsers = (callback) => Il(ea(database, "users"), callback);
const onGetPosts = (callback) => Il(order(), callback);
const deletePost = (uid) => yl(sa(database, "posts", uid));
const updatePost = (uid, newPostEdit) => gl(sa(database, "posts", uid), {
  post: newPostEdit
});
const updateLikePost = (uid, counterLikes) => gl(sa(database, "posts", uid), {
  likes: Nl(counterLikes)
});
const disLikePost = (uid, counterLikes) => gl(sa(database, "posts", uid), {
  likes: kl(counterLikes)
});
const expressions = {
...
const validate = (name2, value, elementId) => {
  switch (name2) {
    case "user":
...
const Register = (onNavigate2) => {
  ...
  const Home2 = document.createElement("form");
  Home2.setAttribute("id", "home");
  const title = document.createElement("h1");
  title.innerText = "<Laboratorians Music>";
  title.setAttribute("class", "title");
  const welcome = document.createElement("p");
  welcome.innerText = "Reg\xEDstrate para compartir m\xFAsica con la comunidad de Laboratoria.";
  ...
  errorUser.innerText = "El nombre debe tener entre 3 y 16 caracteres, puede contener n\xFAmeros, _ y - .";
  ...
  password.innerText = "Contrase\xF1a";
  ...
  passwordInput.placeholder = "Ingresa tu contrase\xF1a";
  passwordInput.id = "passwordInput";
  passwordInput.setAttribute("type", "password");
  passwordInput.setAttribute("name", "password1");
  const errorPassword = document.createElement("h6");
  errorPassword.innerText = "La contrase\xF1a debe tener entre 6 y 16 caracteres, puede contener n\xFAmeros, _ y - .";
  ...
  passwordInput2.placeholder = "V\xE1lida tu contrase\xF1a";
  passwordInput2.id = "passwordInput2";
  passwordInput2.setAttribute("type", "password");
  const errorPassword2 = document.createElement("h6");
  errorPassword2.innerText = "Ambas contrase\xF1as deben ser iguales";
  ...
  buttonGoogle.innerText = "Reg\xEDstro con Google";
  ...
  haveAccount.innerText = "\xBFYa tienes una cuenta?";
  haveAccount.className = "haveAccount";
  const login = document.createElement("h3");
  login.innerText = "Inicia sesi\xF3n";
  ...
  Home2.appendChild(userDiv);
  Home2.appendChild(emailDiv);
  Home2.appendChild(passwordDiv);
  Home2.appendChild(buttonRegister);
  Home2.appendChild(buttonGoogle);
  Home2.appendChild(divAccount);
  containerImageAndRegister.appendChild(imageRegister);
  containerImageAndRegister.appendChild(Home2);
  register.appendChild(title);
  register.appendChild(welcome);
  register.appendChild(containerImageAndRegister);
  const inputs = Home2.querySelectorAll("input");
  inputs.forEach((input, index) => {
    const messageError = Home2.querySelectorAll("h6")[index].id;
  ...
  Home2.addEventListener("submit", (e) => {
    e.preventDefault();
    registerEmailAndPassword(emailInput.value, passwordInput.value).then((UserCredential) => {
...
const Login = (onNavigate2) => {
  ...
  title.id = "title";
  const welcome = document.createElement("p");
  welcome.innerText = "Inicia sesi\xF3n para compartir m\xFAsica con la comunidad de Laboratoria.";
  ...
  imageEye.setAttribute("id", "imageEye");
  const buttonRegister = document.createElement("button");
  buttonRegister.innerText = "Inicia sesi\xF3n";
  buttonRegister.id = "registerButton";
  const buttonGoogle = document.createElement("button");
  const image = document.createElement("img");
  image.src = "https://andigarcia.com/wp-content/uploads/2020/04/Google-Logo-Fondo-negro.jpg";
  ...
  containerImageAndLogin.appendChild(Home2);
  login.appendChild(title);
  login.appendChild(welcome);
  login.appendChild(containerImageAndLogin);
  Home2.addEventListener("submit", (e) => {
    e.preventDefault();
    loginEmailAndPassword(emailInput.value, passwordInput.value).then(() => {
      onNavigate2("/wall");
      location.reload();
      user();
    }).catch((error) => {
      ...
      if (errorCode === "auth/wrong-password") {
        alert("Contrase\xF1a incorrecta, intentalo de nuevo");
...
const Wall = (onNavigate2) => {
  ...
  closeSesion.innerText = "Cerrar Sesi\xF3n";
  ...
  textStart.innerText = "Comparte tu canci\xF3n o gustos musicales favoritos ac\xE1 abajo y recibe muchos likes:";
  ...
  createPost.innerText = "Crea una publicaci\xF3n";
  const post = document.createElement("textarea");
  post.className = "post";
  post.placeholder = "Hola, la canci\xF3n que escuch\xE9 ayer...";
  ...
  onGetPosts((querySnapshot) => {
    ...
    querySnapshot.forEach((doc) => {
      ...
      messagePosts.setAttribute = "messagePosts";
      ...
      imageLikePosts.addEventListener("click", () => {
        if (likesArray.includes(usersUid)) {
          disLikePost(postId, usersUid).then(() => {
          ...
          updateLikePost(postId, usersUid).then(() => {
            console.log("Apareci\xF3 tu Like");
          }).catch((error) => console.log(error));
      ...
      buttonEditPost.addEventListener("click", () => {
        updatePost(postId, editPost.value).then(() => console.log("tu post fue Editado")).catch((error) => console.log(error));
      });
      imageDeletePosts.addEventListener("click", () => {
        if (window.confirm("\xBFQuieres borrar este post?")) {
          deletePost(postId).then((user2) => {
  ...
  closeSesion.addEventListener("click", () => {
    logOut();
    localStorage.clear();
    onNavigate2("/DEV001-social-network/");
  });
  return wallDiv;
};
const Home = (onNavigate2) => {
  ...
  messageP.innerText = "Inicia sesi\xF3n o reg\xEDstrate para compartir m\xFAsica con la comunidad de Laboratoria";
  ...
  buttonLogin.innerText = "Inicia sesi\xF3n";
  const buttonRegister = document.createElement("button");
  buttonRegister.className = "buttonRegister";
  buttonRegister.innerText = "Reg\xEDstrate";
